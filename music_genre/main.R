...
Sys.setenv(SPOTIFY_CLENT_SECRET = '')
...
data %>%
 ...
 data_split <- initial_split(data, prop = 0.8, strata = music_genre)
...
data_recipe <- recipe(music_genre ~ popularity + danceability + acousticness+
                        energy +
                       tempo +valence,
                      data = data_training) %>%
