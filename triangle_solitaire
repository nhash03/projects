...
(@cwl nhashem) ;replace ??? with your cwl
...
(@template try-catch genrec arb-tree encapsulated)
...
(@template use-abstract-fn encapsulated)
(define (next-boards bd)
  (local [(define (move-possible bd j)
            (cond [(empty? j) empty]
                  [else (if (not (false? (get-cell bd (jump-from
                                                       (first j)))))
                            (if (not (false? (get-cell bd (jump-over
                                                           (first j)))))
                                (if (false? (get-cell bd (jump-to (first j))))
                                    (cons (set-cell
                                           (set-cell
                                            (set-cell bd (jump-from (first j))
                                                      false)
                                            (jump-over (first j)) false)
                                           (jump-to (first j)) true)
                                          (move-possible bd (rest j)))
                                    (move-possible bd (rest j)))
                                (move-possible bd (rest j)))
                            (move-possible bd (rest j)))]))]

    (move-possible bd JUMPS)))
